...
if [ $# -ne 8 ]
then
echo "Requires 8 parameters to be passed to run this script"
exit 0
else
...
aws elb create-load-balancer --load-balancer-name $7 --listeners Protocol=Http,LoadBalancerPort=80,InstanceProtocol=Http,InstancePort=80 --subnets subnet-58436a3c
...
aws ec2 run-instances --image-id $1 --key-name $2 --security-group-ids $3 --instance-type t2.micro --user-data file://cron.sh --count $5 --placement AvailabilityZone=us-west-2a --iam-instance-profile Name="$6"
